...
export const ButtonWithLoader:FunctionComponent<IButtonWithLoader> = ({label, loading, disabled, type}) => {
    return (
        <button 
...
export const AddButton:FunctionComponent<IAddButton> = ({title, margin, onClick, disabled}) => {
        ...
        <button 
            ...
            className={
                `${bg} ${hoverStyle} text-white font-bold ${focusStyle} px-2 py-2 text-center inline-flex ` +
                    `items-center ${margin && margin} rounded`
            }
            onClick={(e)=>{onClick(e);}}    
